...
import { ensureString, forget, isMobileSize, isValidPhone, strippedPhone, fetchUser, devLog, preprocessFile } from '../../common/utils';
...
export const ProfileComponent: React.FC = () => {
  ...
  const updateEntering = (newText: string) => {
    dirty();
    setEntering(newText);
  }
  ...
  const syncUser = async () => {
    ...
    syncImage(data);
    if (data?.household) {
      setEntering(data.household.enteringInstructions);
    }
  ...
  const deleteAccount = async () => {
    ...
    invalidateUser();
    router.replace('/(secure)/(home)/sign-out');
  }
    ...
    <BaseScrollLayout>
      ...
      <VStack className="max-w-[440px] w-full" space="md">
        <VStack className="w-full">
          ...
          />
          <VStack>
            <Text>Entering Instructions</Text>
            <Text>What code or keys are required to bring packages into the house</Text>
            <Input
              variant="outline"
              size="md"
              isReadOnly={readOnly}
            ...
            >
              <InputField
                value={entering ? entering : ''}
                onChangeText={updateEntering}
                inputMode="text"
                testID="entering"
                accessibilityLabel="Entering Instructions"
                accessibilityHint="How does a neighbor put packages into the house"/>
            </Input>
          </VStack>
        </VStack>
        {readOnly ? null : (
        ...
        <HStack>
          {readOnly ? (
            ...
            : (<Button
              className="bottom-button shadow-md rounded-lg m-1"
              style={{flex: 1}}
              isDisabled={!isDirty || mobileErrorMessage.length > 0}
